...
<div>
    <div class="container mx-auto py-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:space-x-4 justify-center mb-8 gap-4">
            <input type="text" wire:model.debounce.500ms="search" placeholder="Cari judul buku..." class="border rounded-full px-6 py-3 w-full md:w-1/3 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition shadow">
            <select wire:model="filterCategory" class="border rounded-full px-4 py-2 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">Semua Kategori</option>
                @foreach($categories as $category)
                    <option value="{{ $category->id }}">{{ $category->name }}</option>
                @endforeach
            </select>
            <select wire:model="filterAuthor" class="border rounded-full px-4 py-2 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="bg-white dark:bg-gray-900 shadow-xl rounded-2xl overflow-hidden flex flex-col h-full border border-gray-100 dark:border-gray-800 hover:scale-105 hover:shadow-2xl transition-transform duration-300 group relative">
                    ...
                    <div class="p-6 flex-1 flex flex-col justify-between">
                        ...
                        <div class="mt-6 flex justify-between items-center">
                            <span class="text-lg font-bold text-blue-600 dark:text-blue-400">Stok: {{ $book->stock }}</span>
